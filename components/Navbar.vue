          ...
          <div class="hidden sm:ml-6 sm:block">
            <div class="flex space-x-4">
              <NuxtLink v-for="item in navigation" :key="item.name" :to="item.to"
                :class="[item.current ? 'bg-gray-900 text-white' : 'text-gray-300 hover:bg-gray-700 hover:text-white', 'rounded-md px-3 py-2 text-sm font-medium hover:cursor-pointer']">{{ item.name }}</NuxtLink>
            </div>
          </div>
        </div>
      </div>
...
const navigation = [
  ...
  { name: 'Menu', to: '/menus' },
  { name: 'Kasir', to: '/cashier' },
  { name: 'Transaksi', to: '/transactions' },
]
</script>