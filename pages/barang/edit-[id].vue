<template>
  <div class="container mx-auto xl:px-20 my-8">
    ...
    <div class="mt-8 w-full max-w-sm p-6 bg-white border border-gray-200 rounded-lg shadow-lg mx-auto">
      ...
      <div v-else>
        <form class="space-y-6" action="" method="POST" autocomplete="off" enctype="multipart/form-data" v-on:submit.prevent="ubahBarang">
          <div>
            <img v-bind:src="`http://localhost:8000/storage/images/` + barang.gambar" alt="" class="w-40 mx-auto ring-1 ring-slate-800">
          </div>
          <div>
          ...
          <div class="flex justify-between">
            ...
            <button type="submit"
                class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 rounded-lg px-5 py-2.5">Ubah</button>
          </div>
        </form>
      </div>
    </div>    
...
async function getDataBarang(id) {
...
async function ubahBarang() {
  ...
  try {
    ...
    formData.append('gambar', barang.value.gambar ? barang.value.gambar : null)
    formData.append('_method', 'PATCH')

    const response = await $fetch(`${config.public.apiBase}/api/barang/${barangId.value}`, {
      method: 'POST',
      body: formData,
      headers: {
        'Accept': 'application/json',
